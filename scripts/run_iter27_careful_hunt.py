#!/usr/bin/env python
"""
Iteration 27 — careful research hunt (protected live systems untouched).

Hunts unused markets on high-data live leagues (EPL / Bundesliga / La Liga /
Serie A / Primeira) plus remaining registered leagues (MLS, Austria) if
predictions exist. Does not re-promote already-live cells. Does not lower the bar.
...
OUT = ROOT / "experiments" / "iter27"
OUT.mkdir(parents=True, exist_ok=True)

# Unused markets on leagues we already trust, plus remaining registered leagues.
HUNT_LEAGUES = {
    "EPL": None,
    "Bundesliga": None,
    "LaLiga": None,
...
# Already live — never treat as a new promotion even if the cell still clears.
ALREADY_LIVE = {
    "EPL_ou25_under_2.0-4.0_e8",
    "EPL_ou25_over_1.6-2.5_e10",
    "Bundesliga_ou25_under_1.7-2.5_e10",
    "LaLiga_1x2_H_e8_max1.8",
    "SerieA_1x2_A_e3_max2.0",
    "PrimeiraLiga_ah_e12_max1.9",
}
...
def is_already_live(r: dict) -> bool:
    return str(r.get("tag") or "") in ALREADY_LIVE
...
def main() -> int:
    ...
    for key in HUNT_LEAGUES:
        info = get_league(key)
        aligned_p = ROOT / "data" / "interim" / info["aligned"]
        pred_p = find_preds(key)
        if pred_p is None:
            lines.append(f"## {key}: SKIP — no predictions.parquet")
            lines.append("")
            continue
        if not aligned_p.is_file():
            lines.append(f"## {key}: SKIP — no aligned file `{aligned_p.name}`")
            lines.append("")
            continue
    ...
    cleared = [r for r in all_rows if clears_bar(r) and not is_already_live(r)]
    live_hits = [r for r in all_rows if is_already_live(r)]
    lines += [
    ...
    if not cleared:
        lines.append("None — keep hunting; do not wire new live packs. Bar unchanged.")
    lines += [
        "",
        "## Already-live cells (not new promotions)",
        "",
    ]
    if not live_hits:
        lines.append("No already-live tags appeared in this hunt grid (tag formatting may differ).")
    for r in live_hits:
        lines.append(
            f"- `{r['tag']}` n={r.get('n')} ROI={100*(r.get('roi') or 0):+.1f}% "
            f"t={r.get('t_stat') or float('nan'):.2f} — leave frozen"
        )
    lines += [
        "",
        "## Live systems check",
        "",
        "No pack rules modified in this iteration. All six protected systems untouched.",
        "",
    ]
    report = "\n".join(lines)
